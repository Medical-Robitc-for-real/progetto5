...
Straight_length_1 = 95
Straight_length_2 = 115
Straight_length_3 = 145
Curved_length_1 = 30
Curved_length_2 = 40
Curved_length_3 = 55
Tube_radius_1 = 0.85
Tube_radius_2 = 0.75
Tube_radius_3 = 0.65
Radius_curvature_1 = 90
Radius_curvature_2 = 100
Radius_curvature_3 = 115
...
class KeyBoardController(Sofa.Core.Controller):
    def __init__(self, *args, **kwargs):
        ...
        print("BENVENUTO")
        print("Selezionare il tubo da controllare (Ctrl + 0/1/2)")
        print("Successivamente selezionare il tipo di moto: traslazione in avanti o indietro (Ctrl + arrow up/down) - rotazione in verso orario o antiorario (Ctrl + arrow left /right)")
    ...
    def generate_workspace(self, filename):
        ...
        total_points = n_ins1 * n_rot1 * n_ins2 * n_rot2 * n_ins3 * n_rot3
...
def createScene(rootNode):
    ...
    surface_mesh = rootNode.addChild("SurfaceMesh")
    surface_mesh.addObject('MeshOBJLoader', name='loader', filename="/home/emanu/SOFA/v25.06.00/plugins/ModelOrderReduction/share/sofa/examples/ModelOrderReduction/others/caduceus/mesh/snake_body.obj")

    # Contenitore topologico (usa i dati del loader)
    surface_mesh.addObject('MeshTopology', src='@loader')

# Oggetto meccanico (posizioni nodi)
    surface_mesh.addObject('MechanicalObject', name="dofs", template="Vec3d")

# Modello grafico (OpenGL)
    surface_mesh.addObject('OglModel', src='@loader', color=[0.8, 0.6, 0.2, 1.0], translation=[-25, 58, 0])

# Mapping tra i dati meccanici e la grafica
    #surface_mesh.addObject('IdentityMapping', input='@dofs', output='@visual')

    #   -creazione mesh volumetrica

   # volumetric_mesh = surface_mesh.addChild("VolumetricMesh")
   # volumetric_mesh.addObject('Tetrahedralize', name='TetGen', method='Delaunay', elementSize=1.0)
   # volumetric_mesh.addObject('MechanicalObject', name='dofs', template='Vec3d')
   # volumetric_mesh.addObject('UniformMass', totalMass=1.0)
   # volumetric_mesh.addObject('TetrahedronFEMForceField', name='FEM', youngModulus=1e5, poissonRatio=0.3)

    #   -visualizzazione mesh

    # volumetric_mesh.addObject('OglModel', color=[0.7,0.3,0.3,1.0])
    ...
    visuOgl_1.addObject('OglModel', name='Visual', color='0 1 1 1', quads="@../Container_1.quads") #1 0 0 1
    ...
    visuOgl_2.addObject('OglModel', name='Visual', color='1 1 0 1', quads="@../Container_2.quads") #0 1 0 1
...
def main():
    ...
    Sofa.Gui.GUIManager.Init("volumetric_snake", "qglviewer")
